...
type Actions =
	...
	| { type: 'changeCurrNodeData'; data: getNodeData_type }
...
type API = {
	...
	changeCurrNodeData: (val: getNodeData_type) => void;
...
export const ViewDataProvider: React.FC<{
	...
	const api = useMemo(() => {
		...
		const changeCurrNodeData = (currNode_data: getNodeData_type) =>
			dispatch({ type: 'changeCurrNodeData', data: currNode_data });
	...
	return (
		<ViewContextState.Provider value={viewState}>
			<ViewContextAPI.Provider value={api}>
				{children}
			</ViewContextAPI.Provider>
