	...
	return (
		<>
			<div {...props} className={'split-pane-top'} ref={topRef}>
				...
				/>
				<div style={{ paddingTop: UNIT * 3.5 + 'em' }}>
					{props.children}
				</div>
			</div>
			<Divider
...
export const SplitPaneLeft = (props: any) => {
	...
	useEffect(() => {
		if (!clientWidth || clientWidth == 0) {
			setClientWidth(1.5 * topRef.current.clientWidth);
		}
	}, [currTab]);
...
export const SplitPaneRight = (props: any) => {
	return (
		<div {...props} className='split-pane-right z-10'>
			{props.children}
		</div>
	);
};
