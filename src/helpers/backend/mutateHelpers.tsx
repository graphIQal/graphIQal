...
export const updateNode = (
	...
	switch (type) {
		...
		case 'resize':
			...
			if (newVal.tag === 'top') {
				const val =
					graphNodes[nodeID].y.low +
					graphNodes[nodeID].height.low -
					newSize[1];
			...
			if (newVal.tag === 'left') {
				const val =
					graphNodes[nodeID].x.low +
					graphNodes[nodeID].width.low -
					newSize[0];
				if (Number.isFinite(val)) {
...
export const addNode = (
	...
	let newData = { ...context.nodeData_Graph };
	newData[id] = {
		content: [],
		type: '',
	};
...
export const changeConnectionType = (
	...
	for (let node in viewContext.nodeData_Graph) {
		if (Object.keys(viewContext.nodeData_Graph).includes(node)) {
			console.log(
				'node ' + JSON.stringify(viewContext.nodeData_Graph[node])
			);
		}
	}
};
