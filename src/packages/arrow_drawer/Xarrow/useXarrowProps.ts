...
const parseLabels = (label: xarrowPropsType['labels']): labelsType => {
  let parsedLabel: any = { start: null, middle: null, end: null };
  if (label) {
    if (typeof label === 'string' || React.isValidElement(label))
      parsedLabel.middle = label;
    else {
      for (let key in label) {
        parsedLabel[key] = (label as any)[key];
      }
    }
  }
  return parsedLabel;
...
const parseDashness = (dashness: any, props: any) => {
  let dashStroke = 0,
    dashNone = 0,
    animDashSpeed,
    animDirection = 1;
...
const parseEdgeShape = (svgEdge: any): svgCustomEdgeType => {
 ...
 * @param updateRef
 */
const withUpdate = (propVal: any, updateRef: any) => {
...
const parseNumWithUpdatePos = (userProp: any, _: any, updatePos: any) =>
  withUpdate(Number(userProp), updatePos);
const parseNum = (userProp: any) => Number(userProp);

const parsePropsFuncs: Required<{ [key in keyof xarrowPropsType]: Function }> =
  {
    start: (userProp: any) => getElementByPropGiven(userProp),
    end: (userProp: any) => getElementByPropGiven(userProp),
    startAnchor: (userProp: any, _: any, updatePos: any) =>
      withUpdate(parseAnchor(userProp), updatePos),
    endAnchor: (userProp: any, _: any, updatePos: any) =>
      withUpdate(parseAnchor(userProp), updatePos),
    labels: (userProp: any) => parseLabels(userProp),
    color: noParse,
    lineColor: (userProp: any, propsRefs: any) => userProp || propsRefs.color,
    headColor: (userProp: any, propsRefs: any) => userProp || propsRefs.color,
    tailColor: (userProp: any, propsRefs: any) => userProp || propsRefs.color,
    ...
    gridBreak: (userProp: any, _: any, updatePos: any) =>
      withUpdate(parseGridBreak(userProp), updatePos),
    // // gridRadius = strokeWidth * 2, //todo
    dashness: (userProp: any, propsRefs: any) =>
      parseDashness(userProp, propsRefs),
    headShape: (userProp: any) => parseEdgeShape(userProp),
    tailShape: (userProp: any) => parseEdgeShape(userProp),
    showXarrow: noParse,
...
const parseGivenProps = (props: any, propsRef: any) => {
  for (let [name, val] of Object.entries(props)) {
    propsRef[name] = (parsePropsFuncs as any)?.[name]?.(val, propsRef);
  }
  return propsRef;
};
...
const useXarrowProps = (
  ...
  useEffect(
    () => {
      //@ts-ignore
      propsRefs['start'] = parsePropsFuncs?.['start']?.(
        //@ts-ignore
        curProps['start'],
        propsRefs,
    ...
    //@ts-ignore
    propsDeps['start'].map((name: any) => userProps[name])
  );
  useEffect(
    () => {
      //@ts-ignore
      propsRefs['end'] = parsePropsFuncs?.['end']?.(
        //@ts-ignore
        curProps['end'],
        propsRefs,
        shouldUpdatePosition
    ...
    //@ts-ignore
    propsDeps['end'].map((name: any) => userProps[name])
  );
  // useEffect(
  //   () => {
  //     //@ts-ignore
  //     propsRefs['color'] = parsePropsFuncs?.['color']?.(
  //       //@ts-ignore
  //       curProps['color'],
  //       propsRefs,
  //       shouldUpdatePosition
  ...
  //   },
  //   //@ts-ignore
  //   propsDeps['color'].map((name: any) => userProps[name])
  // );
