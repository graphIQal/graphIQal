...
// return relative,abs
export const xStr2absRelative = (
  str: any
): { abs: number; relative: number } => {
  ...
  //added by me idk what it does
  return { abs: 0, relative: 0 };
};
...
export const getShortestLine = (sPoints: t1[], ePoints: t1[]) => {
  ...
  closestPair = { chosenStart: sPoints[0], chosenEnd: ePoints[0] };
  sPoints.forEach((sp) => {
    ePoints.forEach((ep) => {
      d = dist(sp, ep);
      if (d < minDist) {
...
export const getSvgPos = (svgRef: React.MutableRefObject<any>) => {
  ...
  let { left: xarrowElemX, top: xarrowElemY } =
