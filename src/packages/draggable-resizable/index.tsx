...
export class Rnd extends React.PureComponent<Props, State> {
  ...
  onDragStart(e: RndDragEvent, data: DraggableData) {
    ...
    if (this.props.bounds === 'parent') {
      ...
      const top =
        -(parentTop - parent.offsetTop * scale - bodyRect.top) / scale;
      const right =
    ...
    this.setState({
      bounds: {
        ...
        bottom:
          top +
          (boundary.offsetHeight - this.resizable.size.height) -
          offset.top,
    ...
    if (this.props.bounds) {
      ...
      if (
        ...
        if ((hasLeft || hasTop) && this.resizable) {
          const max =
            (selfLeft - boundaryLeft) / scale + this.resizable.size.width;
  ...
  onResize(
    ...
    if (
      newPos.x !== this.draggable.state.x ||
      newPos.y !== this.draggable.state.y
    ) {
  ...
  render() {
    ...
    return (
      ...
      <Resizable
        ...
        maxWidth={
          this.state.resizing ? this.state.maxWidth : this.props.maxWidth
        }
        maxHeight={
          this.state.resizing ? this.state.maxHeight : this.props.maxHeight
        }
        grid={resizeGrid}
