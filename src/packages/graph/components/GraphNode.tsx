          ...
          <div className='bg-base_white h-full'>
            ...
            <Cube className='absolute right-sm top-sm' size={'1.5em'} />
				...
				{collapsed ? (
					...
					<div>
						{getTypedConnections(viewContext, id, 'HAS')?.map(
							(nodeID, i) => {
								let { icon, color } =
									viewContext.nodeData_Graph[nodeID];
								let childTitle =
									viewContext.nodeData_Graph[nodeID].title;

								return (
									<div key={i} className=' bg-opacity-50'>
										<h3
											className={
												'absolute top-0 left-0 text-white p-1 bg-' +
												color
											}
										>
											{title}
