    ...
    <ul>
      ...
      <div className='flex flex-col bg-blue-400 '>
        ...
        <TextButton
          text={'Test Button'}
          onClick={async () => {
            console.log(GenerateCypher([addBlockOnNodeCreation, returnCypher]));
          }}
        />
        <TextButton
          text={'Delete user'}
          onClick={async () => {
            console.log(await deleteUser({ username, password, email }));
          }}
        />
        <div>care</div>
        <TextButton
          ...
          onClick={async () => {
            ...
            await fetch('/api/general/load_resources').then((res) => {
              res.json().then((json) => {
                console.log(json);
              });
            });
        ...
        <TextButton
          ...
          onClick={async () => {
            console.log('add resources');
            await fetch('/api/general/delete_resources').then((res) => {
              res.json().then((json) => {
                console.log(json);
              });
            });
